...
export default {
        ...
        line_items: [
          {
            price:
              item === "1"
                ? Bun.env.STRIPE_PRICE_ID_1
                : item === "2"
                ? Bun.env.STRIPE_PRICE_ID_2
                : item === "3"
                ? Bun.env.STRIPE_PRICE_ID_3
                : "",
            quantity: 1,
          },
        ],
  ...
  successTtransaction: async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.status(200).send(`<h1 >Pago aceptado 👌👌👌👌👌 </h1>`);
    } catch (error) {
  ...
  canceledTRansaction: async (req: Request, res: Response) => {
    ...
    try {
      ...
      await client.query("COMMIT");
      res.status(200).send(`<h1 >Algo ha ido mal 😞😞😞😞😞</h1>`);
    } catch (error) {
      await client.query("ROLLBACK");
      res.status(500).json({ message: "error" });
    } finally {
    ...
    try {
      ...
      if (response.rows.length > 0) {
        ...
        const token = jwt.sign(
          { id: user.rows[0].id, email: user.rows[0].email, suscribed: true },
          Bun.env.JWT_SECRET!,
          { expiresIn: "24h" }
        );
